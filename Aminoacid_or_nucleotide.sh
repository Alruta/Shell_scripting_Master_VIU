...
while [[ $mas == "si" ]]
	do
		...
		for n in $file ;
			do
				if [[ $n != "A" && $n != "G" && $n != "C" && $n != "T" ]] ; # Condiciones para que sea una              													secuencia aminoacídica
					then
						# Es una secuencia aminoacidica
						a=1 # cambio en la variable
						break
				fi
			done
		...
		if [[ $a != 0 ]] ;
			then
				echo "El archivo introducido es una secuencia aminoacídica" # a=1 al pasar los criterios de if en el  													       for loop anterior
			else
				echo "El archivo introducido es una secuencia nucleotídica" # a=0 al no pasar los criterios
		fi
